...
const files = walkSync(sourceDirectory)
    .map(file => ({
        filePath: file,
...
files.forEach(file => {
    mkDirByPathSync(`${path.resolve(destinationDirectory)}${path.sep}${file.fileDatePath}`);
    moveFiles(file.filePath, `${path.resolve(destinationDirectory)}${path.sep}${file.fileDatePath}${path.sep}${path.basename(file.filePath)}`);
});
